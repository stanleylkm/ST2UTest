...
namespace SuperTiled2Unity.Editor
{
    ...
    public class TilesetLoader
    {
        ...
        public bool LoadFromXml(XElement xTileset)
        {
            ...
            ProcessWangsets(xTileset);

            return true;
        }

        private void ProcessWangsets(XElement xTileset)
        {
            ...
            if (xWangsets != null)
            {
                ...
                {
                    ProcessWangset(xWangset);
                }
            }
        }
        private void ProcessWangset(XElement xWangset)
        {
            string name = xWangset.GetAttributeAs("name", "Wangset");
        }
        ...
        private void BuildTilesetFromCollection(XElement xTileset, AtlasBuilder atlas)
        {
            ...
            foreach (var xTile in xTileset.Elements("tile"))
            {
                ...
                var customProperties = CustomPropertyLoader.LoadCustomPropertyList(xTile.Element("properties"));
                Vector4 border = Vector4.zero;
                foreach (var item in customProperties)
                {
                    if (item.m_Name == "Border")
                    {
                        var vector4String = item.m_Value;
                        var lbrt = vector4String.Split(',');
                        border.x = float.Parse(lbrt[0]);
                        border.y = float.Parse(lbrt[1]);
                        border.z = float.Parse(lbrt[2]);
                        border.w = float.Parse(lbrt[3]);
                    }
                }
                ...
                if (xImage != null)
                {
                    ...
                    atlas.AddTile(tileIndex, tex2d, rcSource, border);
                }
            }
        }
        ...
        private void ProcessAnimationElement(SuperTile tile, XElement xAnimation)
        {
            ...
            var interval = tile.GetPropertyValueAsFloat("randominterval", 0);
            var animations = new AnimationBuilder(fps, interval);
