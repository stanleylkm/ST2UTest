...
namespace SuperTiled2Unity.Editor
{
    public static class GameObjectExtensions
    {
        ...
        public static void BroadcastProperty(this GameObject go, SuperObject so, CustomProperty property)
        {
            ...
            foreach (var comp in components)
            {
                ...
                if (method != null)
                {
                    if(method.GetParameters().Length == 1)
                        ...
                        method.Invoke(comp, new object[2] { so, objValue });
                    continue;
                }
                ...
                // Then, private fields
                var csPrivateField = FindPrivateFieldBySignature(comp, property.m_Name);
                if (csPrivateField != null)
                {
                    try
                    {
                        csPrivateField.SetValue(comp, objValue);
                    }
                    catch (ArgumentException)
                    {
                        var stringValue = objValue as string;
                        object enumValue = Enum.Parse(csPrivateField.FieldType, stringValue);
                        csPrivateField.SetValue(comp, enumValue);
                    }
                    continue;
                }
        ...
        private static MethodInfo FindMethodBySignature(MonoBehaviour component, string name, Type paramType)
        {
            return component.GetType().GetMethods(BindingFlags.Instance | BindingFlags.Public).Where(info =>
            {
                ...
                {
                    if (parameters[0].ParameterType != typeof(SuperObject))
                        Debug.LogError($"First parameter of {info.Name} must be SuperObject.");
                    if (parameters[1].ParameterType != paramType)
                        return false;
                }
        ...
        private static FieldInfo FindPrivateFieldBySignature(MonoBehaviour component, string name)
        {
            return component.GetType().GetFields(BindingFlags.Instance | BindingFlags.NonPublic).Where(
                info =>
                !info.IsInitOnly &&
                info.Name == name
                ).FirstOrDefault();
