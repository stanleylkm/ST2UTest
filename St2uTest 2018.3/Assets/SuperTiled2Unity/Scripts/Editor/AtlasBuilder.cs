...
namespace SuperTiled2Unity.Editor
{
    ...
    public class AtlasBuilder
    {
        ...
        public void AddTile(int index, Texture2D texSource, Rect rcSource, Vector4 border = default)
        {
            var atlasTile = new AtlasTile() { Index = index, SourceTexture = texSource, SourceRectangle = rcSource, Border = border };
        ...
        private void Commit()
        {
            ...
            foreach (var t in ordered)
            {
                ...
                border = t.Border;
                //border.x /= t.PreferredRectangle.size.x;
                //border.z /= t.PreferredRectangle.size.x;
                //border.y /= t.PreferredRectangle.size.y;
                //border.w /= t.PreferredRectangle.size.y;

                // Create the sprite with the anchor at (0, 0)
                var sprite = Sprite.Create(t.PreferredTexture2D, t.PreferredRectangle, Vector2.zero,
                    m_TiledAssetImporter.SuperImportContext.Settings.PixelsPerUnit, 0, SpriteMeshType.FullRect, border);
